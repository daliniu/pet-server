...
void MD5::update (uint1 *input, uint4 input_length) {
  ...
  if (finalized){  // so we can't update!
    fprintf(stderr, "MD5::update:  Can't update a finalized digest!\n");
    return;
  }
...
void MD5::finalize (){
  ...
  if (finalized){
    fprintf(stderr, "MD5::finalize:  Already finalized this digest!\n");
    return;
  }
...
bool MD5::raw_digest(unsigned char *s, int len){
  ...
  if (!finalized){
    fprintf(stderr, "MD5::raw_digest:  Can't get digest if you haven't finalized the digest!\n");
...
bool MD5::hex_digest_upcase(char *s, int len){
  ...
  if (!finalized){
    fprintf(stderr, "MD5::hex_digest:  Can't get digest if you haven't finalized the digest!\n");
    return "";
  }
...
bool MD5::hex_digest_lowcase(char *s, int len){
