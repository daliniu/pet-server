  ...
  elseif event == "line" then
    ...
    if (lastsource ~= caller.source) then
      ...
      if file:find("^@") then
		...
		--@todo fix
		--原来取文件路径的方法不对，暂时先这样处理
        local pos = file:find("scripts")
        file = file:sub(pos + 8)
        --if file:find("%./") == 1 then file = file:sub(3)
        --else file = file:gsub('^'..q(basedir), '') luo("sub@@>",file,">>basedir>>",basedir) end
    ...
    --luo("file>>",file,">>line>>",line,">>has_breakpoint>>",tostring(has_breakpoint(file, line)))
...
local function handle(params, client, options)
  ...
  --luo("Handles server commands ",command)
